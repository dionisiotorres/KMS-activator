...
BOOL AesEncryptMessageV5(const BYTE *IV, BYTE *Message, DWORD *MessageSize, DWORD BufferLen)
{
	BOOL success = FALSE;
	HCRYPTPROV hProv = 0;
	HCRYPTKEY hKey = 0;

	success = LAZY_FN("ADVAPI32", CryptAcquireContextW)(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT) &&
		LAZY_FN("ADVAPI32", CryptImportKey)(hProv, (BYTE*)&AesKeyV5Blob, sizeof(AesKeyV5Blob), 0, 0, &hKey) &&
		(IV != nullptr ? LAZY_FN("ADVAPI32", CryptSetKeyParam)(hKey, KP_IV, IV, 0) : TRUE) &&
		LAZY_FN("ADVAPI32", CryptEncrypt)(hKey, 0, TRUE, 0, Message, MessageSize, BufferLen);

	if (hKey)
		LAZY_FN("ADVAPI32", CryptDestroyKey)(hKey);
	if (hProv)
		LAZY_FN("ADVAPI32", CryptReleaseContext)(hProv, 0);

	return success;
}

// KMS V5 AES-128 CBC decryption
BOOL AesDecryptMessageV5(const BYTE *IV, BYTE *Message, DWORD *MessageSize)
{
	BOOL success = FALSE;
	HCRYPTPROV hProv = 0;
	HCRYPTKEY hKey = 0;
	...
	success = LAZY_FN("ADVAPI32", CryptAcquireContextW)(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT) &&
		LAZY_FN("ADVAPI32", CryptImportKey)(hProv, (BYTE*)&AesKeyV5Blob, sizeof(AesKeyV5Blob), 0, 0, &hKey) &&
		(IV != nullptr ? LAZY_FN("ADVAPI32", CryptSetKeyParam)(hKey, KP_IV, IV, 0) : TRUE) &&
		LAZY_FN("ADVAPI32", CryptDecrypt)(hKey, 0, TRUE, 0, Message, MessageSize);
...
void AesEncryptBlock(BYTE *state, const BYTE *RoundKeys)
{
...
void AesDecryptBlock(BYTE *state, const BYTE *RoundKeys)
{
	AddRoundKey(state, RoundKeys[AES_ROUNDS * AES_KEY_BYTES]);
...
BOOL AesEncryptMessageV6(const BYTE *IV, BYTE *Message, DWORD *MessageSize, DWORD BufferLen)
{
...
BOOL AesDecryptMessageV6(const BYTE *IV, BYTE *Message, DWORD *MessageSize)
{
	if (*MessageSize & 0xf)
		return FALSE;
...
// Intern Hash
void Hash(const BYTE *message, BYTE *hash)
{
...
BOOL GetSha256Hash(const BYTE *data, DWORD dataSize, BYTE *Hash)
{
	...
	success = LAZY_FN("ADVAPI32", CryptAcquireContextW)(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT) &&
		LAZY_FN("ADVAPI32", CryptCreateHash)(hProv, CALG_SHA_256, 0, 0, &hHash) &&
		LAZY_FN("ADVAPI32", CryptHashData)(hHash, data, dataSize, 0) &&
		LAZY_FN("ADVAPI32", CryptGetHashParam)(hHash, HP_HASHVAL, Hash, &HashSize, 0);

	if (hHash)
		LAZY_FN("ADVAPI32", CryptDestroyHash)(hHash);
	if (hProv)
		LAZY_FN("ADVAPI32", CryptReleaseContext)(hProv, 0);
...
BOOL GetHmacSha256(const BYTE *pbKey, DWORD dwDataLen, const BYTE *pbData, BYTE *pbHash)
{
	...
	success = LAZY_FN("ADVAPI32", CryptAcquireContextW)(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT) &&
		LAZY_FN("ADVAPI32", CryptImportKey)(hProv, (PBYTE)&HmacKeyblob, sizeof(HMAC_KEYBLOB), 0, 0, &hKey) &&
		LAZY_FN("ADVAPI32", CryptCreateHash)(hProv, CALG_HMAC, hKey, 0, &hHmacHash) &&
		LAZY_FN("ADVAPI32", CryptSetHashParam)(hHmacHash, HP_HMAC_INFO, (PBYTE)&HmacInfo, 0) &&
		LAZY_FN("ADVAPI32", CryptHashData)(hHmacHash, pbData, dwDataLen, 0) &&
		LAZY_FN("ADVAPI32", CryptGetHashParam)(hHmacHash, HP_HASHVAL, pbHash, &dwHmacSize, 0);

	if (hKey)
		LAZY_FN("ADVAPI32", CryptDestroyKey)(hKey);

	if (hHmacHash)
		LAZY_FN("ADVAPI32", CryptDestroyHash)(hHmacHash);
...
BOOL GetRandomBytes(BYTE *RandomBuffer, DWORD RandomBufferLength)
{
	...
	success = LAZY_FN("ADVAPI32", CryptAcquireContextW)(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT) &&
		LAZY_FN("ADVAPI32", CryptGenRandom)(hProv, RandomBufferLength, RandomBuffer);
